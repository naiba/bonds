...
Isso executa o backend Go na `:8080` e o servidor de desenvolvimento Vite na `:5173`. O frontend faz proxy das pedidos de API para o backend automaticamente.
...
::: warning
Nunca edite manualmente ficheiros em `web/src/api/generated/`. Eles são sobrescritos a cada geração.
:::
...
| `make build-all` | Compilar binário único com frontend incorporado |
...
server/                    # Backend Go
  ...
  internal/
    ...
    models/                 # Modelos GORM
    dto/                    # Estruturas de pedido/resposta
    middleware/              # Autenticação JWT, CORS, etc.
    search/                 # Pesquisa de texto completo Bleve
...
web/                       # Frontend React
  src/
    ...
    locales/                # Internacionalização (en.json, zh.json, es.json, fr.json, de.json, pt-PT.json, pt-PT.json)
...
Cada funcionalidade segue: **Handler** (camada HTTP) → **Service** (lógica de negócios) → **DTO** (pedido/resposta) → **Model** (GORM).

- Handlers vinculam pedidos, validam, delegam a serviços e retornam via auxiliares `response.*`
...
Para acessar o Swagger UI, ative o modo de depuração ou alterne em Admin > Definições > Swagger:
```bash
...
> O Swagger UI usa por padrão a flag `DEBUG`, mas pode ser ativado/desativado independentemente na página de Definições de Administração.

## Endpoint MCP

O acesso para agentes de IA é servido pelo endpoint [`/mcp`](/pt-PT/features/ai-agents) nativo do backend. É intencionalmente separado do pipeline de geração Swagger/OpenAPI: alterações apenas no MCP não exigem `make gen-api`, e `/mcp` não está incluído no cliente gerado da API do frontend.
