...
大部分配置已从环境变量迁移到数据库存储。管理面板提供 Web UI 来配置：
...
多个个性化设置表支持重排序。你可以通过界面上的按钮将性别、代词、模板页面、日记模板章节、群组角色或模板页内的模块等项目向上或向下移动。

| 实体 | 可自定义内容 | 作用域 | 支持重排序 |
|------|-------------|-------|-----------|
| `genders` | 性别选项 | 账户级 | 是 |
| `pronouns` | 代词选项 | 账户级 | 是 |
| `address-types` | 地址类型标签 | 账户级 | 是 |
| `pet-categories` | 宠物类别 | 账户级 | 是 |
| `contact-info-types` | 联系方式类型 | 账户级 | 是 |
| `relationship-types` | 关系类型定义 | 账户级 | 是（子类型） |
| `templates` | 联系人页面模板 | 账户级 | 是（页面） |
| `modules` | 模块配置 | 账户级 | 是（页面模块） |
| `currencies` | 货币偏好 | 账户级 | 否（启用开关） |
| `religions` | 宗教选项 | 账户级 | 是 |
| `call-reasons` | 通话原因分类 | 账户级 | 是（子原因） |
| `gift-occasions` | 礼物场合类型 | 账户级 | 是 |
| `gift-states` | 礼物追踪状态 | 账户级 | 是 |
| `post-templates` | 日记模板 | 账户级 | 是（章节） |
| `group-types` | 联系人分组类型 | 账户级 | 是（角色） |
| `task-statuses` | 自定义任务状态 | 账户级 | 是 |
