...
1. 使用邮箱和密码注册。
2. **邮箱验证**：系统会向注册邮箱发送一封验证邮件。用户必须点击邮件中的链接激活账号，之后才能创建或访问 Vault。
   - 如果验证邮件丢失或过期，用户可以在验证提示页面上点击重新发送按钮获取新链接。
   - 当在账户设置中更新邮箱地址时，也需要重新进行邮箱验证。
3. 登录返回 JWT 令牌。
4. 令牌通过 `Authorization: Bearer <token>` 请求头发送。
5. 令牌 24 小时后过期（可通过 `JWT_EXPIRY_HRS` 配置）。
6. 7 天内可刷新令牌（可通过 `JWT_REFRESH_HRS` 配置）。
...
1. 输入邮箱和密码。服务器返回 `requires_two_factor: true` 和临时令牌。
...
启用两步验证后，**DAV 的密码认证将被禁止**。DAV 客户端（CardDAV/CalDAV）必须使用[个人访问令牌](/zh/features/more#个人访问令牌)替代密码。这遵循了 Google、Apple、Fastmail 的安全模型。仅泄露密码无法通过 DAV 绕过两步验证访问你的数据。

启用两步验证后配置 DAV 客户端：

1. 前往 **设置 > API 令牌** 创建新令牌。
2. 在 DAV 客户端中，用户名填写你的**邮箱**，密码填写**令牌**（以 `bonds_` 开头）。
...
在新增备份标志持久化之前注册的通行密钥无法恢复其原始的 backup-eligible 值，因此
同步类通行密钥（iCloud 钥匙串、Google 密码管理器、1Password 等）在重新注册前会持续
登录失败。请在设置，安全中删除并重新注册该通行密钥。
...
在管理面板中配置。启用后，登录页面会显示「使用 GitHub 登录」或「使用 Google 登录」按钮。
