...
test.describe("Contact Modules - Notes", () => {
  test("should create a note", async ({ page }) => {
    ...
    await noteModal.getByPlaceholder(/title/i).fill("Test Note Title");
    await noteModal
      .getByRole("textbox", { name: /write your note/i })
      .pressSequentially("This is a test note body");
  ...
  test("should delete a note", async ({ page }) => {
    ...
    const noteModal = page
      ...
      .filter({ hasText: /add note/i });
    await expect(noteModal).toBeVisible({ timeout: 5000 });
    await noteModal.getByPlaceholder(/title/i).fill("Delete Me");
    await noteModal
      .getByRole("textbox", { name: /write your note/i })
      .pressSequentially("Note to delete");
...
test.describe("Contact Modules - Addresses", () => {
  ...
  test("should add and delete an address", async ({ page }) => {
    ...
    const addressItem = addressCard
      .locator(".ant-list-item")
      .filter({ hasText: "742 Evergreen Terrace" });
    await addressItem.locator('[aria-label="delete"]').click();
    const deleteResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/addresses") &&
    ...
    await expect(
      addressCard.getByText("742 Evergreen Terrace"),
...
test.describe("Contact Modules - Pets", () => {
  test("should create a pet", async ({ page }) => {
    ...
    const petModal = page.locator(".ant-modal").filter({ hasText: /add pet/i });
    ...
    const responsePromise = page.waitForResponse(
      (resp) =>
        resp.url().includes("/pets") && resp.request().method() === "POST",
    );
...
test.describe("Contact Modules - Calls", () => {
  test("should log a call", async ({ page }) => {
    ...
    const reasonField = modal
      .locator(".ant-form-item")
      .filter({ hasText: /call reasons/i });
    await reasonField.locator(".ant-select").click();
    await page
      .locator(".ant-select-dropdown:visible .ant-select-item-option")
      .filter({ hasText: /just to say hello/i })
      .click();

    const responsePromise = page.waitForResponse(
      (resp) =>
        resp.url().includes("/calls") && resp.request().method() === "POST",
    );
    await page.locator(".ant-modal-footer .ant-btn-primary").click();
    const resp = await responsePromise;
    expect(resp.status()).toBeLessThan(400);
    ...
    await expect(callsCard.getByText("Just to say hello")).toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe("Contact Modules - Loans", () => {
  test("should create a loan and toggle settled", async ({ page }) => {
    ...
    const createResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/loans") && resp.request().method() === "POST",
    );
    await page.locator(".ant-modal-footer .ant-btn-primary").click();
  ...
  test("should create an item loan with quantity and due date", async ({
    page,
  }) => {
    ...
    await page
      .locator(
        ".ant-picker-dropdown:visible .ant-picker-cell:not(.ant-picker-cell-disabled):not(.ant-picker-cell-today)",
      )
      .first()
      .click();
    ...
    await expect(
      loansCard.getByText(/Due [A-Z][a-z]{2} \d{1,2}, \d{4}/),
...
test.describe("Contact Modules - Goals", () => {
  test("should create a goal", async ({ page }) => {
    ...
    const responsePromise = page.waitForResponse(
      (resp) =>
        resp.url().includes("/goals") && resp.request().method() === "POST",
    );
    await goalsCard.getByRole("button", { name: /^add$/i }).click();
    const resp = await responsePromise;
    expect(resp.status()).toBeLessThan(400);
...
test.describe("Contact Modules - Contact Information", () => {
  test("should create a contact info entry", async ({ page }) => {
    ...
    const infoModal = page
      .locator(".ant-modal")
      .filter({ hasText: /add contact information/i });
...
test.describe("Contact Modules - Contact Information Delete", () => {
  test("should add and delete a contact info entry", async ({ page }) => {
    ...
    const infoItem = infoCard
      .locator(".ant-list-item")
      .filter({ hasText: "+1-555-0199" });
...
test.describe("Contact Modules - Tasks Toggle", () => {
  ...
  test("should create task, toggle it, and see it in completed section", async ({
    page,
  }) => {
...
test.describe("Contact Modules - Labels", () => {
  test("should add and remove a label from a contact", async ({ page }) => {
    ...
    await expect(
      labelsCard.locator(".ant-tag").filter({ hasText: "e2e-test-label" }),
    ).toBeVisible({ timeout: 10000 });

    // Delete the label from the contact
    const deleteResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/labels") && resp.request().method() === "DELETE",
    );
    await labelsCard
      .locator(".ant-tag")
      .filter({ hasText: "e2e-test-label" })
...
test.describe("Contact Modules - Important Dates", () => {
  test("should add and delete an important date", async ({ page }) => {
    ...
    const modal = page
      .locator(".ant-modal")
      .filter({ hasText: /important date/i });
    await expect(modal).toBeVisible({ timeout: 5000 });

    await modal.getByLabel(/label/i).fill("Graduation Day");

    const dateFormItem = modal
      .locator(".ant-form-item")
      .filter({ hasText: "Date" })
      .last();
